...
- **Adding a recorded value is a 3-place edit.** To log a new measurement you must update `initialise()` in `app.py` (create its `record` list), `csvData()` in `chibio_control_helpers.py` (add to both `fieldnames` and `row` — they must stay the same length or the header is skipped), and `downsample()` if it should be downsampled.
