...
- [ ] **Help the user choose fluorescence settings from their sample.** Today FP setup is fully manual: excitation LED + emission bands + gain are picked from hardcoded defaults via `SetFPMeasurement` (`app.py:415`), with no guidance. The raw-measurement building blocks already exist but stop at raw data — `GetSpectrum` (`app.py:624`) reads one AS7341 spectrum, and `CharacteriseDevice` (`app.py:~700`) sweeps every LED × power × emission band into a full excitation–emission matrix but only dumps it to `characterisation_data_<M>.txt` with no analysis or UI. **Explore** adding a layer on top that, from a scan of the actual sample, identifies excitation/emission peaks and recommends the best Excite LED + Base/Emit bands + gain (and flags saturation/weak-signal) — so choosing optimal FP settings is guided, not guesswork. Pairs well with the auto-ranging-gain item above. Not yet implemented; scope/UX to be decided.
