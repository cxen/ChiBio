...
The Run 0 vials were still in M0-M3 (WT in M0/M1/M3, **sterile medium in M2**, ~29 h old, heater
and stirrer off), which makes the rig a **known-answer test**: every vial is non-fluorescent, so
the correct answer for all four reactors is "no fluorophore". Full record in `labnotes.md`.

**What it settled.** The fluorescence assist recommended `LEDI(550)→nm583/nm620` for all four
reactors *including the sterile tube*, with the reported signal ordered by turbidity (37.7 /
269.6 / 275.9 / 516.2). That corrects the standing "biomass autofluorescence" diagnosis — a tube
with no cells earns the same pick, so the dominant term is the 550 nm LED's 105 nm-FWHM red tail
leaking into those bands, multiplied by scatter. **Matched non-fluorescent reference subtraction
is now shipped and device-verified**, and returns the true negative on the live cultures.

**Numbers now measured on this rig, not assumed:**

| | |
|---|---|
| Scan-to-scan repeatability (unchanged sample) | **0.4 %** median per-cell CV |
| Residual after subtracting a matched WT reactor | **6-8 %** (same density), 14-16 % (1.8x apart) |
| Residual using the **sterile** tube as the reference | **20-22 %** — not a substitute for a matched culture |
| sigma_device across four WT reactors at matched OD | **CV 5.1-23.9 %** by channel |
| Per-reactor OD blank spread, same medium | **1.35x** (11651-15764 counts, CV 12.3 %) |
| emit/base ratio shift across the x10→x6 gain change | **-1.5 to -7.8 %** (not exactly gain-invariant) |

**Two things for the bench, both in `labnotes.md`'s fill-in list:** `external_air_temp` reads
exactly **36.25 °C on all four reactors** with the heaters off and the cultures at ~26 °C (four
sensors agreeing to the digit is not a measurement — it feeds no control loop, but do not trust
the column); and the **sterile blank's transmission has fallen 11.7 %** since it was blanked,
which is the one tube whose contents should not change.

**Also observed:** cells settle measurably *within* one 3-replicate OD window — transmission
rises 5-10 % across the three reads in every reactor holding cells and is flat (0.2 %) in the
...
- [x] **`recommend_fp_settings` is fooled by the excitation leak and scatter, which scale with biomass.** *(Fixed + device-verified 2026-08-13 via option (a), matched non-fluorescent reference subtraction.)* **The 2026-07-18 diagnosis below was half right and is corrected here: it is not mainly cellular autofluorescence.** With the Run 0 vials still in the rig (WT in M0/M1/M3, **sterile medium in M2**), all four reactors were handed the identical `LEDI(550)→nm583/nm620` pick and the reported `signal` was ordered by turbidity: **sterile 37.7, WT 269.6 / 275.9, densest WT 516.2**. A tube with no cells in it earns the same recommendation, so the dominant term is the 550 nm LED's own red tail (105 nm FWHM) leaking into nm583/nm620, with cell scatter multiplying it. Fix shipped: `recommend_fp_settings(eem, reference)` picks on the **residual** after subtracting a scaled matched non-fluorescent EEM; the scale is the median of the per-cell ratios so one real FP cell cannot drag the fit. Measured floors — WT-vs-WT at matched density **6-8%**, at 1.8x density **14-16%**, sterile-vs-culture **20-22%** (a sterile tube is NOT a substitute for a matched culture), against **0.4%** scan-to-scan repeatability. Threshold set at 25% of the subtracted background. Known-answer test on the live cultures: M0 referenced against M3 (scale 0.97) returns **no recommendation**, the true answer; clearing it returns the old pick, now labelled `unverified`. Without a reference the pick is preserved but carries `confidence: 'unreferenced'` and says in the UI that a sterile tube of medium earns the same answer. `test_fluorescence.py` sections F/G, built on the four measured EEMs.

  *Original diagnosis (2026-07-18):* **`recommend_fp_settings` is fooled by biomass autofluorescence.** Both reactors returned the *identical* recommendation `excite LEDI(550) → emit nm583/nm620`, and the reported `signal` scaled with OD (80 @ OD 0.34 on M0 vs 103 @ OD 0.40 on M1) — i.e. it locked onto green-excited cellular **autofluorescence** common to both, not the FP (GFP emits ~509/nm510, YFP ~527/nm550; neither at 583). Root cause: the Stokes-shift rule filters excitation *scatter* but not autofluorescence, and the recommender picks the strongest Stokes-valid EEM cell — which at OD ≳0.3 with dim single-copy FPs is autofluorescence. The genuine FP cells are weak: GFP LEDB(457)→nm510 ≈ 10.6, YFP LEDD(523)→nm550 ≈ 19.2. Fixes to weigh: (a) subtract a WT / no-FP autofluorescence blank (needs a control reactor — not currently available); (b) constrain/weight excitation toward the fluorophore's known band; (c) require the FP-region cell to beat the autofluor cell by a margin before recommending it; ~~(d) only trust the auto-recommendation at higher density where FP:autofluor improves~~ — **DISPROVEN 2026-07-18**: re-scanned at true plateau (OD 1.54 M0 / 1.37 M1) and it STILL recommended ex550→nm583/620, identical for both fluorophores (signal 230/258, ~equal despite different OD) — autofluorescence and scatter scale with biomass just like the FP, so density never improves the ratio. The fix must be (a)/(b)/(c), not "wait for denser". This is the "tune the Stokes cutoff + noise-floor against a known fluorophore" knob from the `sensor-failure-semantics`/fluorescence notes — now with data across three densities showing the failure mode.
...
- [x] **First-class staged media recipes + inducer/pressure ramps.** *(Built + device-verified 2026-08-13, `chibio_schedule.py`.)* A schedule is a list of `(at_h, item, target, ramp)` stages the control loop follows; `ramp` on a stage interpolates linearly from the previous stage for that item, which is the Wenk medium-composition ramp written down, while plain steps are the Joshi reservoir swap. Routes `/SetSchedule/<M>` (JSON body) and `/ScheduleOnOff/<value>/<M>`; a `schedule_stage` CSV column and a `schedule_stage` events record mean a scheduled change logs itself (the events `exp_time` comes from the last completed cycle, so use the `at_h` in the detail for the precise moment). UI: an editor whose rows are client-owned with a dirty flag, so the 1 s poll cannot wipe what you are typing, and the in-force stage is marked only while the editor matches what is actually running.

  **Design constraints found while building, all enforced in `validate_schedule`:** Pump1/Pump2 are computed by `RegulateOD` every cycle, so scheduling them while OD/chemostat control is on is refused (two writers on one actuator) — **Pump3/Pump4 are free and are the inducer/second-reservoir channel**; targets outside an item's declared range are refused rather than clamped; and **Stir cannot be ramped**, because `SetOutput`'s Stir branch hard-starts the motor at full power for 1.5 s on *any* target change, so a ramp would kick it every tick for the length of the ramp.

  Verified on the device against a live reactor: all four validation refusals returned 400 with the offending stage named; a scheduled LED step came on at its 18 s mark and off at 50 s through real I2C; a ramped setpoint interpolated 30.26 → 31.05 → 31.85 → 32.65 → 33.44 → 34.00 and held; `schedule_stage` appeared in the CSV (64 columns) and three transitions in the events sidecar. `test_schedule.py` (6 sections) + `test_schedule_ui.py` (UI/backend channel-list drift guard). **Two defects caught before shipping:** `applied` was a per-item index while the CSV column and the UI row marker read it as a global one, and the `M == "0"` "current UI device" sentinel was unhandled in every entry point — a 500 for every button in the panel, which the device probe missed because it passed `M2` explicitly.
